...
final class ENFRiskCalculation: ENFRiskCalculationProtocol, Codable {
	...
	private(set) var riskLevelPerDate: [Date: ENFRiskLevel] = [:]
	...
	private(set) var riskLevel: ENFRiskLevel = .low
	...
	func calculateRisk(
		...
		riskLevelPerDate = normalizedTimePerDate.compactMapValues { normalizedTime -> ENFRiskLevel? in
