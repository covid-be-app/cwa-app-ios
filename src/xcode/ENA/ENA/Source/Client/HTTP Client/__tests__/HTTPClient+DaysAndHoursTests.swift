...
final class HTTPClientDaysAndHoursTests: XCTestCase {
	...
	func testAvailableDays_Success() {
		...
		HTTPClient.makeWith(mock: stack).availableDays(region: .belgium) { result in
			switch result {
	...
	func testAvailableHours_NotFound() {
		...
		HTTPClient.makeWith(mock: stack).availableHours(day: "2020-05-12", region: .belgium) { result in
			switch result {
	...
	func testFetchHour_InvalidPayload() throws {
		...
		HTTPClient.makeWith(mock: stack).fetchHour(1, day: "2020-05-01", region: .belgium) { result in
			switch result {
			case .success:
	...
	func testFetchDay_Success() throws {
		...
		HTTPClient.makeWith(mock: stack).fetchDay("2020-05-01", region: .belgium) { result in
			defer { successExpectation.fulfill() }
			switch result {
			case let .success(sapPackage):
				self.assertPackageFormat(for: sapPackage)
